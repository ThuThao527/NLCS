    ...
    <div v-if="showViewModal" class="modal">
      <div class="modal-content">
        ...
        <div class="user-details" v-if="selectedUser">
          ...
          <div class="detail-row">
            <strong>Lần đăng nhập cuối:</strong> {{ selectedUser.lastLogin }}
          </div>
        </div>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="saveUser" v-if="selectedUser">
          ...
          <div class="form-group">
            ...
            <input type="text" id="address" v-model="editForm.address" required />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="addUser">
          ...
          <div class="form-group">
            ...
            <input type="text" id="new-phone" v-model="newUser.phone" required />
          </div>
          <div class="form-group">
            <label for="new-address">Địa chỉ</label>
            <input type="text" id="new-address" v-model="newUser.address" required />
          </div>
          <div class="form-group">
...
const editForm = reactive({ name: '', email: '', phone: '', address: '', status: '' });
const newUser = reactive({ name: '', email: '', phone: '', address: '', status: 'active' });

// Lấy danh sách người dùng từ API
const fetchUsers = async () => {
  try {
    const response = await apiClient.get('/api/UserList', {
      params: {
    ...
    users.value = response.data.helpers.map(user => ({
      ...
      name: user.FullName,
      email: user.Email,
      phone: user.PhoneNumber, // Thêm số điện thoại
      address: user.Address, // Thêm địa chỉ
      status: user.IsDeleted === null ? 'active' : 'inactive', // Hoạt động nếu IsDeleted là null
      avatar: user.AvatarUrl || 'https://randomuser.me/api/portraits/men/1.jpg',
...
const saveUser = async () => {
  if (selectedUser.value) {
    try {
      ...
      await apiClient.put(`/api/users/${selectedUser.value.id}`, updatedData);
...
const deleteUser = async () => {
  if (selectedUser.value) {
    try {
      await apiClient.delete(`/api/users/${selectedUser.value.id}`);
